...
在较慢主机上，全新的 PostgreSQL 数据卷初始化可能需要一分钟以上。生成的 Compose
为真实的 `pg_isready` 健康检查提供有界的 90 秒启动宽限期，随后仍按 2 秒间隔、
5 秒单次超时和 15 次失败上限检查。`make migrate-up` 会等待就绪；如果 PostgreSQL
始终不健康就会失败，不会关闭持久性或返回假成功。
