...
test('actual npm tarball installs without dev dependencies and its mapped bin generates away from source', async (t) => {
  ...
  for (const file of packed.files) {
    assert.doesNotMatch(file, /(^|\/)(node_modules|\.git|(?!\.env\.example$)\.env[^/]*|\.trellis|tests)(\/|$)|\.tsbuildinfo$|lock\.yaml$/);
  ...
  for (const asset of requiredAssets) {
    ...
    const expectedGenerated = relative === 'api/go.mod'
      ? Buffer.from(installedBytes.toString('utf8').replace(/^module example\.com\/temvia\/api/, `module ${modulePath}`))
      : relative.endsWith('.go')
        ? Buffer.from(installedBytes.toString('utf8').replaceAll('example.com/temvia/api', modulePath))
        : installedBytes;
    assert.deepEqual(await fs.readFile(generated), expectedGenerated, `Tarball-to-generated bytes: ${relative}`);
  }
